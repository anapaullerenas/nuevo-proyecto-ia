...
import {
  composeStaticCreative,
  TextCompositionError,
  type LogoSource,
  type TextCompositionVerification,
...
export async function POST(request: NextRequest) {
  ...
  if (!user) {
    return NextResponse.json(
      { error: "Inicia sesión para generar estáticos." },
      { status: 401 },
    );
  }
  ...
  if (!body.brandId || !body.format || !body.funnelStage) {
    return NextResponse.json(
      { error: "Faltan datos de la ficha para generar." },
      { status: 400 },
    );
  }
  ...
  const { data: storedArchetype } =
    !curatedArchetype && ficha.arquetipo
      ? await supabase
          ...
          .maybeSingle()
      : { data: null };
  const archetype = (curatedArchetype ||
    storedArchetype) as StaticArchetype | null;
  const activeArchetypes = variants > 1 ? CURATED_STATIC_FORMATS : [];
  const candidateArchetypes = [
    archetype,
    ...activeArchetypes.filter(
      (item) =>
        item.id !== archetype?.id &&
        normalizeStage(item.stage) === normalizeStage(body.funnelStage),
    ),
    ...activeArchetypes.filter((item) => item.id !== archetype?.id),
  ].filter(
    (item, index, list) =>
      Boolean(item) &&
  ...
  try {
    ...
    for (let index = 0; index < variants; index += 1) {
      const variationIndex =
        index + Math.max(0, Number(body.variantOffset) || 0);
      const variantArchetype =
        candidateArchetypes[variationIndex] ||
        candidateArchetypes[
...
function adaptBriefToArchetype(
  ...
  const zones = (structure.estructura || structure.zones || {}) as Record<
    string,
    string
  >;
  const art = (structure.art_direction_default || {}) as Partial<
    StaticBrief["art_direction"]
  >;
  return normalizeStaticBrief({
    ...ficha,
    arquetipo: archetype.id,
    arquetipo_label: archetype.label_visible,
    composicion: {
      zona_superior:
        zones.zona_superior || zones.top || ficha.composicion.zona_superior,
      zona_media:
        zones.zona_media ||
        zones.middle ||
        zones.center ||
        ficha.composicion.zona_media,
      zona_inferior:
        zones.zona_inferior || zones.bottom || ficha.composicion.zona_inferior,
    },
    art_direction: { ...ficha.art_direction, ...art },
  });
}
...
async function inspectStaticImage({
  ...
  const fallback: StaticQa = applyProgrammaticQa(
    {
      ...
      veredicto: "aprobada",
      razon:
        "QA visual no disponible; se aplicó la verificación raster programática.",
    },
    ficha,
    textVerification,
    composeError,
  ...
  const expectedCta =
    ficha.cta_usage === "none"
      ? "NO DEBE HABER CTA"
      : cleanQaText(ficha.cta) || "NO DEBE HABER CTA";
  const content: Array<Record<string, unknown>> = [
    {
      type: "text",
      text: `Audita la PRIMERA imagen, que es el PNG FINAL YA COMPUESTO del anuncio ${format}; no evalúes una base previa.
...
Si CUALQUIER booleano es false, veredicto debe ser "regenerar".`,
  ...
  if (productAsset && supabase) {
    const { data } = await supabase.storage
      .from(productAsset.bucket_id)
      .download(productAsset.storage_path);
    if (data)
      content.push({
        type: "image_url",
        image_url: {
          url: `data:${productAsset.mime_type || "image/png"};base64,${Buffer.from(await data.arrayBuffer()).toString("base64")}`,
          detail: "high",
        },
      });
  }
  try {
          ...
          json_schema: {
            ...
            schema: {
              ...
              properties: {
                ...
                texto_presente_y_legible: { type: "boolean" },
                sin_formas_vacias: { type: "boolean" },
                zona_segura_ok: { type: "boolean" },
                sin_artefactos: { type: "boolean" },
                cumple_reglas_de_oro: { type: "boolean" },
                look_disenador: { type: "boolean" },
                veredicto: { type: "string", enum: ["aprobada", "regenerar"] },
                razon: { type: "string" },
              },
              required: [
                "producto_fiel",
                "texto_correcto",
                "texto_presente_y_legible",
                "sin_formas_vacias",
                "zona_segura_ok",
                "sin_artefactos",
                "cumple_reglas_de_oro",
                "look_disenador",
                "veredicto",
                "razon",
              ],
        ...
        messages: [
          {
            role: "system",
            content:
              "Eres control de calidad visual estricto para anuncios DTC. La primera imagen es siempre el PNG final compuesto; la segunda, si existe, es el producto fuente de verdad.",
          },
          { role: "user", content },
        ],
      }),
    ...
    return applyProgrammaticQa(
      JSON.parse(data.choices?.[0]?.message?.content || "{}") as StaticQa,
      ficha,
      textVerification,
      composeError,
    );
...
function applyProgrammaticQa(
  qa: StaticQa,
  ficha: StaticBrief,
  verification: TextCompositionVerification,
  composeError: string | null,
): StaticQa {
  const expectsLayeredText =
    ficha.text_render_mode === "layered" &&
    [
  ...
  const rasterTextOk =
    !expectsLayeredText || (verification.passed && !composeError);
  const result = {
    ...qa,
    texto_presente_y_legible: Boolean(
      qa.texto_presente_y_legible && rasterTextOk,
    ),
    sin_formas_vacias: Boolean(
      qa.sin_formas_vacias && verification.emptyShapeCount === 0,
    ),
  };
  const checks = [
    result.producto_fiel,
    result.texto_correcto,
    result.texto_presente_y_legible,
    result.sin_formas_vacias,
    result.zona_segura_ok,
    result.sin_artefactos,
    result.cumple_reglas_de_oro,
    result.look_disenador,
  ];
  if (checks.some((check) => !check)) {
    const programmaticReason = !rasterTextOk
      ? `La verificación raster no encontró el texto compuesto${composeError ? `: ${composeError}` : ""}.`
      : "";
    return {
      ...result,
      veredicto: "regenerar",
      razon: [result.razon, programmaticReason].filter(Boolean).join(" "),
    };
  }
...
async function generateImage({
  ...
  if (orderedAssets.length && supabase) {
    ...
    for (const [index, asset] of orderedAssets.entries()) {
      const { data: fileBlob, error } = await supabase.storage
        ...
        .download(asset.storage_path);
      if (error || !fileBlob) {
        if (asset.kind === "product_photo")
          throw new Error(
            "No pudimos leer la foto original del producto. Vuelve a subirla antes de generar.",
          );
        continue;
      }
      files.push({
    ...
    if (files.length) {
      try {
        ...
        if (productAsset) {
          ...
          throw new Error(
            "No generamos una sustitución falsa del producto. Revisa la foto original e intenta de nuevo.",
          );
        }
        throw error;
      }
  ...
  if (productAsset)
    throw new Error(
      "No pudimos usar la foto original del producto. La generación fue detenida para proteger su fidelidad.",
    );
...
async function composeGeneratedImage(
  base64: string,
  ficha: StaticBrief,
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      const composed = await composeStaticCreative({
        base,
        ficha,
        logoSources,
      });
      return {
        image: composed.buffer.toString("base64"),
        composeError: null,
  ...
  const verification =
    lastError instanceof TextCompositionError
