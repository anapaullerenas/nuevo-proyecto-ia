...
export default async function AdminPage() {
  ...
  return (
    <main className="admin-page">
      ...
      <section className="admin-server-controls">
        <header>
          <span className="eyebrow">Soporte admin v3 · directo servidor</span>
          <h1>Agregar usuarios y reparar accesos</h1>
          <p>
            Estos controles no dependen del panel interactivo. Si alguien no
            puede entrar, usa “Reparar acceso”: activa perfil, acceso manual y
            wallet de 600 créditos.
          </p>
        </header>
        <div>
          <form action="/api/admin" method="post">
            ...
            <label>
              Nombre opcional
              <input name="fullName" placeholder="Nombre" />
            </label>
            <input type="hidden" name="note" value="Alta manual desde controles servidor" />
          ...
          <form action="/api/admin" method="post">
            ...
            <input type="hidden" name="note" value="Reparación desde controles servidor" />
          ...
          <form action="/api/admin" method="post">
            ...
            <input type="hidden" name="reason" value="Abono manual desde controles servidor" />
            <button type="submit">Abonar saldo manual</button>
          </form>
        </div>
        <small>
          Base Supabase usada: auth.users, profiles, credit_wallets,
