  ...
  return (
    <form className="onboarding-form deep-brand-form" onSubmit={handleSubmit}>
      ...
      <section className="brand-brief-fields">
        <div className="form-grid"><Field label="Nombre de marca" value={form.name} onChange={(value) => update("name", value)} required placeholder="Ej. Skinglow" /><Field label="Sitio o Instagram" value={form.website} onChange={(value) => update("website", value)} placeholder="https:// o @usuario" /><Field label="Categoría" value={form.category} onChange={(value) => update("category", value)} required placeholder="Belleza, moda, cursos…" /><label>Quién crea el contenido<select value={form.content_owner} onChange={(event) => update("content_owner", event.target.value)}><option value="owner">La persona/dueña lo crea</option><option value="team">Tiene equipo interno</option><option value="agency">Agencia o freelancer</option><option value="mixed">Mixto</option></select></label></div>
        ...
        <TextField label="Oferta principal" value={form.offer} onChange={(value) => update("offer", value)} required placeholder="Producto, mecanismo, precio, bonos, garantía y transformación." />
